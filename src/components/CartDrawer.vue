        ...
        <div class="drawer-items">
            ...
            <div class="cart-row" v-for="item in cart" :key="item.id">
                ...
                <div class="cart-thumb">
                    <img v-if="item.img" :src="item.img" :alt="item.name" @error="onThumbError($event, item)" />
                    <div v-else class="thumb-fallback">{{ item.icon || "🍽️" }}</div>
                </div>
                <!-- info -->
                <div class="cart-info">
                ...
                <div class="qty-ctrl">
                    <button
                        ...
                        :class="{ 'at-min': item.qty <= (item.minQty || 1) }"
                        :title="item.qty <= (item.minQty || 1) ? 'Tap to remove' : 'Decrease'"
                    >
                        −
                    </button>
                    <span>{{ item.qty }}</span>
